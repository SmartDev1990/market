...
export function shortenAddress(address, chars = 4) {
  if (!address) return '';
  if (address === '0x4B2e0F18810c8905E3C376aa05f414c571053663') {
...
export const getRandomIPFS = (tokenURI, justURL = false, isFallback = false) => {
  ...
  if (isFallback) {
    if (tokenURI.includes('ipfs://')) {
      return `https://tomato-tough-squid-756.mypinata.cloud/ipfs/${tokenURI.split('ipfs://')[1].replace(/([^:]\/)\/+/g, "$1")}`;
    }
    else {
      return `https://tomato-tough-squid-756.mypinata.cloud/ipfs/${tokenURI.split('ipfs/')[1]}`;
    }
  }
  try {
    if (
